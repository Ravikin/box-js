...
def __anal(s_name):										# step 2 (funkcja nic nie zwraca bo wynik analizy jest w plikach)
	...
	if substring in out:									# W przypadku gdy petla [:66]for zakonczyla sie z wynikiem negatywnym (tzn gdy doszlo do timeoutu) to wypisujemy ta informacje
...
def __init(plik):										# step 0
	...
	try:
		...
		with open ("jsanal.tmp",'w') as file:						# Otwieramy tymczasowy plik (zapisujemy podany obiekt do pliku)
			file.write(plik.read())							# zapisujemy podany obiekt w celu dalszej analizy (box-js pracuje na pliku)
